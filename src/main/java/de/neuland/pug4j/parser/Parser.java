...
//        }
        ...
        throw new PugParserException(filename, lexer.getLineno(), templateLoader, token);
    }
    ...
    private Node parseMixinBlock() {
        ...
        if (this.inMixin == 0) {
            throw new PugParserException(filename, lexer.getLineno(), templateLoader, "Anonymous blocks are not allowed unless they are part of a mixin.");
        }
    ...
    private Node parseCase() {
        ...
        while (!(peek() instanceof Outdent)) {
            if (peek() instanceof Comment) {
                ...
                throw new PugParserException(filename, lexer.getLineno(), templateLoader, "Unexpected token \"" + this.peek() + "\", expected \"when\", \"default\" or \"newline\"");
            }
        }
        expect(Outdent.class);
        node.setBlock(block);
...
//            ++i;
    ...
    private Token expect(Class expectedTokenClass) {
        ...
        if (t.getClass().equals(expectedTokenClass)) {
            ...
            throw new PugParserException(filename, lexer.getLineno(), templateLoader, expectedTokenClass, t.getClass());
        }
    }
